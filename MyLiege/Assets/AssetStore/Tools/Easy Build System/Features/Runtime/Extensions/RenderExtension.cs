                    ...
                    if (renderers[i] != null)
                    {
                        if (!ignoreRenderers.Contains(renderers[i]))
                        {
                            for (int x = 0; x < renderers[i].sharedMaterials.Length; x++)
                            {
                                if (GraphicsSettings.currentRenderPipeline)
                                {
                                    renderers[i].sharedMaterials[x].SetColor("_BaseColor", color);
                                }
                                else
                                {
                                    renderers[i].sharedMaterials[x].SetColor("_Color", color);
                                }
                            }
                        }
                    }
            ...
            else
            {
                for (int i = 0; i < renderers.Length; i++)
                {
                    if (renderers[i] != null && renderers[i].enabled)
                    {
                        if (!ignoreRenderers.Contains(renderers[i]))
                        {
                            Material[] copySharedMaterials = renderers[i].sharedMaterials;
                            ...
                            renderers[i].sharedMaterials = copySharedMaterials;
                        }
                    }
                }
            }
