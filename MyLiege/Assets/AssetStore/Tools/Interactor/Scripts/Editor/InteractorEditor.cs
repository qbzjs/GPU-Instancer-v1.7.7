    ...
    public class InteractorEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button(new GUIContent("Auto", "Set Rules With Auto Configure For Humanoid Avatars, Select The Effector Type First"), _autoButton))
            {
                ...
                Animator _animator;
                if (_animator = _script.GetComponentInChildren<Animator>())
                {
                    AdjustCollider(_animator);
                    Transform effector;
                    ...
                    if ((int)type == 0)
                    {
                        effector = _animator.GetBoneTransform(HumanBodyBones.Spine);
                        if (effector)
                        {
                    ...
                    {
                        effector = _animator.GetBoneTransform(HumanBodyBones.LeftUpperArm);
                        if (effector)
                        {
                            Transform lowerArm = _animator.GetBoneTransform(HumanBodyBones.LeftLowerArm);
                    ...
                    {
                        effector = _animator.GetBoneTransform(HumanBodyBones.RightUpperArm);
                        if (effector)
                        {
                            Transform lowerArm = _animator.GetBoneTransform(HumanBodyBones.RightLowerArm);
                    ...
                    {
                        effector = _animator.GetBoneTransform(HumanBodyBones.LeftUpperLeg);
                        if (effector)
                        {
                            Transform lowerLeg = _animator.GetBoneTransform(HumanBodyBones.LeftLowerLeg);
                            Transform foot = _animator.GetBoneTransform(HumanBodyBones.LeftFoot);
                    ...
                    {
                        effector = _animator.GetBoneTransform(HumanBodyBones.RightUpperLeg);
                        if (effector)
                        {
                            Transform lowerLeg = _animator.GetBoneTransform(HumanBodyBones.RightLowerLeg);
                            Transform foot = _animator.GetBoneTransform(HumanBodyBones.RightFoot);
                    ...
                    {
                        Debug.LogWarning("Auto configure only possible for these Effector Types: Body, Left Hand, Right Hand, Left Foot, Right Foot", _script);
                    }
                }
                else
                {
                    Debug.LogWarning("Auto Configure could not find Animator component on this object or any of children of it. You can set the effector rules manually.", _script);
                }
            }
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.EndVertical();
