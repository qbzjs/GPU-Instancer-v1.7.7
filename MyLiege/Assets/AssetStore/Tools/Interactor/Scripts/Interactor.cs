...
namespace razz
{
    ...
    public class Interactor : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (!(_interactorIK = GetComponent<InteractorIK>()) && !InteractorAi)
            {
        ...
        public void StartStopInteraction(InteractorObject interactorObject, bool pathfind)
        {
            for (int i = 0; i < intObjComponents.Count; i++)
            {
                if (intObjComponents[i].interactorObject == interactorObject)
        ...
        {
            if (!click && selectedByUI < intObjComponents.Count && StartInteractorAI(intObjComponents[selectedByUI].interactorObject)) return;
        ...
        {
            if (InteractorAi && intObj.aiTarget && !intObj.used)
            {
                if (!intObj.Reached)
                {
                    if (intObj.GetOtherUseableEffector(10) == -1 || intObj.aiOnly)
                    {
                        InteractorAi.StartPathfinding(intObj);
                        return true;
                    }
                }
