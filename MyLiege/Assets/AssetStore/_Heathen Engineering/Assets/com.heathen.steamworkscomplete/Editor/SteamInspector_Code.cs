...
namespace HeathenEngineering.SteamworksIntegration.Editors
{
    public class SteamInspector_Code : EditorWindow
    {
        ...
        {
            var version = SessionState.GetString("com.heathen.steamworks-version", "3.2");
        ...
        private void OnGUI()
        {
            ...
            if (steamAppId != null)
            {
                steamAppId.text = GetSteamAppIdText();
            }
            else
            {
            ...
            if (Application.isPlaying)
            {
                ...
                else if (HeathenEngineering.SteamworksIntegration.SteamSettings.HasInitializationError)
                {
